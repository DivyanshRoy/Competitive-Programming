//Factorise N very very fast
//Replace isPrime method if range of N<=10^6 (IT IS SLOW)
#include<bits/stdc++.h>
using namespace std;
...
int main()
{
    ...
    tmp=n;
    fact(n);
    map<llui,int>::iterator iter;
    for(iter=factors.begin();iter!=factors.end();iter++)
    {
        cout<<"prime factor: "<<iter->first<<" power: "<<iter->second<<endl;
    }
    return 0;
}
